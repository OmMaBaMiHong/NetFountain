...
| `level1_pool` | 一级池：从多个供应商（91HTTP / default_http，一个配置文件 `global`+`providers` 多供应商配置）拉取 IP、代理可达性测试（每供应商独立拉取器与测试管线，拉取与测试解耦多 worker）、按 `proxy_url` 去重入共享环形池、TTL/容量淘汰、对外查询 API。 | 8000 |
